var deviceModule1 = angular.module('deviceModule.controllers');


deviceModule1.controller('devramlCtrl', function($http, $scope, $rootScope, ENV, deviceModuleService, gatewayModuleService, ENV, $cookieStore, $location, $filter, $timeout, $window, $uibModal, $state, toaster, CustomMessages, AclService, $mdDialog, imageConstant, $uibModal) {
	$scope.selDeviceList=[]
    ...
    $scope.getRAMLDev = function() {
        $scope.getRAMList = [];
        deviceModuleService.getRAMLDeviceTemplate().then(function(data) {
    ...
    $scope.EditSaveData = function() {
        ...
        $scope.deviceIds = []
        for(var i=0;i<$scope.selDeviceList.length;i++){
        	$scope.deviceIds.push($scope.selDeviceList[i].id)
        }
        $scope.finalData = {
            "devicename": $scope.editrml.devicename,
            "description": $scope.editrml.deviceDesc,
            "keywords": $scope.editrml.devicekeywords.split(","),
            "orgid": $scope.editrml.orgid,
            "definitions": $scope.saveproperties,
            "subdefinitions": $scope.savesubproperties,
            "item": [],
...
deviceModule.controller('addDevicesCtrl', function($http, $scope, $rootScope, ENV, deviceModuleService, gatewayModuleService, ENV, $cookieStore, $location, $filter, $timeout, $window, $uibModal, $state, toaster, CustomMessages, AclService, $mdDialog, imageConstant, dsparam, $uibModalInstance) {
    $scope.selDeviceList = []
    ...
    $scope.setClickedRow = function(index, device) {
        ...
        if ($scope.selDeviceList.length == 0) {
            $scope.selDeviceList.push(device)
        } else {
            if ($scope.selDeviceList.indexOf(device) == -1) {
                $scope.selDeviceList.push(device)
            } else {
                var index = $scope.selDeviceList.indexOf(device);
                $scope.selDeviceList.splice(index, 1)
            }

        }
        console.log($scope.selDeviceList)
    }
    $scope.clearDevicemodal = function() {
        $uibModalInstance.close()
    }
    $scope.saveData = function() {
        $rootScope.$broadcast('seldevicelist', $scope.selDeviceList)
