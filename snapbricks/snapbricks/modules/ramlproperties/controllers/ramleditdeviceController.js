...
ramlpropertiesModule5.controller('ramleditDevicepropertiesCtrl', function ($scope, $rootScope, ramlpropertiesModuleService,  ENV,  $cookieStore,$location,$filter,$timeout, $window, $state,toaster,$mdDialog,CustomMessages) {
	...
	$scope.EditSaveData = function()
	{
		...
		angular.forEach($scope.editrml.defination, function(defination,skey) {
			...
			$scope.finalData = {"devicename":$scope.editrml.devicename,"description":$scope.editrml.deviceDesc,"keywords":$scope.editrml.devicekeywords.split(","),"orgid":$scope.editrml.orgid,
								"definitions":$scope.saveproperties,"subdefinitions":$scope.savesubproperties, "item":[]}; //"item":$scope.editrml.defination
